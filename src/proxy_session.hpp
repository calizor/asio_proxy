...
class ProxySession : public std::enable_shared_from_this<ProxySession> {
   ...
   private:
    void read_http_header() {
        auto self = shared_from_this();
        http::async_read_header(client_socket_, client_buffer_,
                                *parser_,  // у optional перегружен оператор *; same as parser_.value();
    ...
    void on_header_read() {
        ...
        if (req.method() == http::verb::connect) {
            ...
            if (size_t pos = target_domain_.find(':');
                pos != std::string::npos) {  // if statement with initializer, pos lives in if and else block
    ...
    void send_connect_ok() {
        ...
        asio::async_write(client_socket_, asio::buffer(*response), [self](boost::system::error_code ec, std::size_t) {
            if (!ec) {
    ...
    void process_request() {
        ...
        if (current_req_.method() == http::verb::get && cache_ != nullptr) {
            auto cached_body =
                cache_->get(cache_key_);  // Предполагаем, что метод get() возвращает std::optional<string>
            if (cached_body) {
    ...
    void perform_target_handshake() {
        ...
        target_ssl_stream_->async_handshake(ssl::stream_base::client, [self](boost::system::error_code ec) {
            if (!ec)
    ...
    void forward_request_to_target() {
        ...
        http::async_write(*target_ssl_stream_, current_req_, [self](boost::system::error_code ec, std::size_t) {
            if (!ec)
                self->read_response_from_target();
            else
                self->close();
        });
    }
    ...
    void forward_response_to_client() {
        ...
        http::async_write(*client_ssl_stream_, *res_ptr, [self, res_ptr](boost::system::error_code ec, std::size_t) {
